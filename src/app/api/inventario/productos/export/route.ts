...
interface Row {
  ...
  precio_costo: string | number | null;
  precio_mayorista: string | number | null;
  precio_minorista: string | number | null;
  ...
  try {
    const { rows } = await pool.query<Row>(
      `SELECT p.nombre, p.sku, col.nombre AS color_nombre, tal.nombre AS talla_nombre,
              ...
              p.unidad_medida, p.costo_promedio, p.precio_costo, p.precio_mayorista, p.precio_minorista, p.precio_venta,
    ...
    const numOrEmpty = (v: string | number | null) => (v == null ? "" : Number(v));
    ...
    const buf = buildXlsxBuffer<Row>(rows, [
      ...
      { header: "PRECIO_COSTO", value: (r) => numOrEmpty(r.precio_costo), width: 14 },
      { header: "PRECIO_MAYORISTA", value: (r) => numOrEmpty(r.precio_mayorista), width: 14 },
      { header: "PRECIO_MINORISTA", value: (r) => numOrEmpty(r.precio_minorista), width: 14 },
