 ...
 *
 * Emisión asíncrona: en vez de correr borrador → xml → firmar → enviar
 * cliente-side (30-35s bloqueando la caja), este endpoint:
 *
 *   1) Asegura que exista `factura_electronica` en 'borrador' (mismo handler
 *      que POST .../sifen/borrador — cero cambio semántico).
 *   2) Inserta un `sifen_job` en estado 'pendiente'.
 *   3) Responde 202 { started, job_id, ya_habia_activo } inmediatamente.
 *
 * El worker toma el Job y ejecuta xml/firmar/enviar/consulta_lote llamando
 * DIRECTAMENTE las funciones del server (sin fetch loopback, sin cookies).
 * `empresa_id` + `data_schema` quedan congelados en el Job para que el worker
 * no tenga que re-resolver el tenant.
 *
 ...
 * se responde el estado actual para que la UI muestre el resultado.
 */
export async function POST(
  ...
  try {
    ...
    // 1) Asegurar borrador reutilizando el handler compartido. Si ya existe,
    //    devuelve el registro tal cual (idempotente). Si SIFEN no está
    //    configurado/activo, replica el mismo error 400 que /borrador — la UI
    //    ya sabe manejarlo.
    ...
    if (!borradorRes.ok) {
      ...
      return NextResponse.json(
        errorResponse(borradorJson.error ?? "No se pudo asegurar el borrador."),
