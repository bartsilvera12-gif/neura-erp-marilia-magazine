...
/**
 * Repositorio de la cola SIFEN (`sifen_jobs`).
 *
 * El Job congela `empresa_id` + `data_schema` al momento del encolado para que
 * el worker opere sin re-resolver la empresa desde JWT/cookie — no tiene sesión.
 */
...
function toDto(row: RawJobRow): SifenJobDTO {
  ...
  return {
    ...
    intentos_log: Array.isArray(row.intentos_log) ? (row.intentos_log as SifenJobIntento[]) : [],
...
// =============================================================================
// Worker — claim / complete / fail / reclaim
// =============================================================================
 ...
 *
 * No usamos `FOR UPDATE SKIP LOCKED` (PostgREST no lo expone).
 */
 ...
 * Devuelve el nuevo estado ('pendiente' si reprogramado, 'error' si cerrado).
 */
export async function registrarIntentoYPlanificar(
  ...
  if (puedeReintentar) {
    // Backoff: intento 1 → 5s, intento 2 → 20s. El caller puede override con
    // backoffOverrideMs para casos específicos (p.ej. 0301 [1264] usa 30s
    // porque suele ser transitorio y necesita que el padrón de SET propague).
    const backoffMs =
