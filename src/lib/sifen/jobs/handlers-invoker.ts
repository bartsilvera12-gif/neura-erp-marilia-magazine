...
/**
 * Puente entre el worker headless y los handlers HTTP-shape de `src/lib/sifen`.
 * Los handlers reciben `(request, params, auth, supabase, opts)` y devuelven
 * `NextResponse`; el worker los invoca con:
 *   - Un `NextRequest` sintético construido sobre una URL loopback. Los handlers
 *     sólo lo usan para leer `?debug=1` — no hacen fetch ni leen headers de
 *     sesión (el auth se pasa por parámetro).
 ...
 *
 * IMPORTANTE: no hay HTTP real, ni red, ni cookies, ni Bearer. `NextRequest`
 * acá es sólo un contenedor tipado.
 */
...
function fakeRequest(): NextRequest {
  // La URL no importa: los handlers sólo leen `request.nextUrl.searchParams`
  // para flags de debug. Origen loopback estándar.
