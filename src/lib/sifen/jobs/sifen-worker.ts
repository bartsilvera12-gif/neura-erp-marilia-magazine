 ...
 *  - Toma UN Job por tick (no paraleliza) — SIFEN es una operación pesada
 *    (firma XML-DSig, SOAP HTTPS a SET). Para volumen alto, escalar a N réplicas.
 *
 ...
 * resuelve dentro de `runSifenJob` a partir de `job.empresa_id`.
 *
 * Server-only: se importa exclusivamente desde `src/instrumentation.ts`.
 */
...
function claimSupabase(): AppSupabaseClient {
  // Service role sobre el schema por defecto (NEURA_CLIENT_SCHEMA). Instancia
  // dedicada monocliente: un solo schema para toda la cola.
...
export function startSifenWorker(): void {
  ...
  const state: WorkerState = { lastReclaimAt: 0 };
