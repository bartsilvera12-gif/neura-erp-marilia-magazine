 ...
 * de rendirse.
 *
 * Un lote en 0361 ("en procesamiento") NO es un error: SET normalmente tarda
 * mucho más que unos minutos. Si se cerrara temprano, el DE quedaría clavado
 * en 'enviado' hasta que un humano apriete "Consultar lote", y con él la venta
 * (que no se puede anular si el DE no está 'aprobado').
 *
 * 60 re-encolados ≈ 48h acumuladas con el backoff creciente de abajo.
 */
 ...
 * Rápido al principio (la mayoría resuelve en segundos) y espaciado después,
 * para no golpear a SET.
 */
...
/**
 * Log estructurado pre-envío a SET. Sirve para diagnosticar rechazos
 * 0301 [1264] («RUC no habilitado») en producción: deja rastro de qué RUC/DV
 * emisor y receptor se está por mandar, en qué ambiente y con qué CDC +
 * `sifen_regeneracion_seq` (para ver si el retry generó un CDC nuevo). No
 * loguea el certificado ni ninguna contraseña.
 */
async function logPreEnvioSifen(
  ...
  try {
    ...
    const { data: cli } = clienteId
      ? await supabase
          .from("clientes")
          .select("id, ruc, documento, tipo_cliente, sifen_receptor_naturaleza, sifen_ti_ope")
          .eq("id", clienteId)
          .maybeSingle()
      : { data: null };
    ...
    console.log(
      `${jobLabel} pre-envio-sifen ${JSON.stringify({
        ...
        receptor_naturaleza: cli?.sifen_receptor_naturaleza ?? null,
        receptor_ti_ope: cli?.sifen_ti_ope ?? null,
        cdc: fe?.cdc ?? null,
...
/**
 * Gate fiscal: sólo se emite si `empresa_sifen_config.activo` es true.
 *
 * Es la única barrera entre la cola y el fisco. Sin esto, una fila de config
 * heredada de otro contribuyente (RUC ajeno + ambiente producción) haría que
 * el worker emita documentos tributarios REALES a nombre de otra empresa.
 * Ante duda (fila ausente, error de lectura) se bloquea.
 */
async function sifenHabilitadoParaEmpresa(
  supabase: AppSupabaseClient,
  empresaId: string
  ...
  if (error) {
    return {
      ok: false,
      motivo: `No se pudo leer la configuración de facturación electrónica: ${error.message}`,
    };
  }
  if (!data || data.activo !== true) {
    return {
      ok: false,
      motivo:
        "Facturación electrónica desactivada: configurá los datos fiscales de la empresa.",
    };
  }
  return { ok: true };
}
 ...
 * estado 'enviado' (ya persistido) y el operador puede darle "Consultar lote"
 * cuando SET responda.
 */
async function cerrarPorSetTimeout(
  ...
  const mensaje =
    ...
    `La factura sigue registrada como enviada en SET; podés usar "Consultar lote" ` +
    `para verificar el resultado cuando SET responda.`;
...
export async function runSifenJob(job: SifenJobDTO): Promise<void> {
  ...
  try {
    // === GATE FISCAL ===
    // Antes de tocar nada: la empresa debe tener SIFEN activo. Es 'config'
    // (no reintentable) → el Job se cierra en 'error' y el operador lo ve.
    const habilitado = await sifenHabilitadoParaEmpresa(supabase, job.empresa_id);
    if (!habilitado.ok) {
      await fallarIntento(
        supabase,
        job,
    ...
    // === ENVIAR ===
    {
      ...
      if (necesitaEnviar) {
        ...
        if (recibe && recibe.loteNoEncolado === true) {
          ...
          // Reintento condicional para 0301 [1264]: en producción este código
          // a veces es transitorio (SET intermitente / caché de padrón
          // desactualizada). En vez de cerrar como rechazado inmediatamente,
          // se le da UN reintento automático con espera de 30s. Si vuelve a
          // fallar, ahí sí se cierra como rechazado definitivo. Sólo aplica a
          // 0301 [1264] (no a rechazos determinísticos como XSD inválido o
          // timbrado vencido) y sólo en el primer intento del Job.
          ...
          if (es0301_1264 && primerIntento) {
            ...
            // Regenerar XML antes del re-encolado: sin esto, el segundo intento
            // reenviaría el mismo XML firmado y SET responde CDC duplicado
            // (0362 [1001]) — aunque haya rechazado el primer envío con 0301,
            // ya registró el CDC. POST /sifen/xml bumpea sifen_regeneracion_seq
            // → CDC nuevo. El Job re-encolado, al ver estado 'generado',
            // ejecuta firmar → enviar por su cuenta.
    ...
    // === CONSULTA-LOTE (polling inline) ===
    {
      ...
      for (let i = 0; i < CONSULTA_INTENTOS_INLINE; i++) {
        ...
        if (!r.ok) {
          ...
          // documento: sólo que no pudimos preguntar. Si contara como intento
          // fatal, a los 2 mataría el Job y la factura quedaría clavada en
          // 'enviado' aunque SET la aprobara después. Se re-encola con el mismo
          // backoff que "SET sigue procesando".
        ...
        if (ultimoEstado === "rechazado") {
          ...
          const detalle = Array.isArray(uc?.detallePorCdc)
            ? (uc?.detallePorCdc as Array<{ grupoRes?: Array<{ dCodRes?: string; dMsgRes?: string }> }>)
            : [];
