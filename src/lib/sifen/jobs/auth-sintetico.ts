 ...
 *
 * Simula el resultado de `getUserAndEmpresa(request)` pero SIN request HTTP: el
 * worker corre en background y no tiene sesión web ni cookies.
 *
 * Los handlers (`handleSifen*Post`) sólo usan `auth.empresa_id`; nunca hacen
 * `auth.getUser()` ni consultan el catálogo. La empresa se resuelve por el
 * `empresa_id` congelado en el `sifen_job` al momento del encolado.
 *
 * Este es el eslabón que garantiza que el worker NO puede resolver otra empresa:
 * `empresa_id` viaja como dato (no como sesión) y el service role filtra por él
 * en cada query.
 */
