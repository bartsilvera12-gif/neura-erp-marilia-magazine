...
/**
 * Genera XML rDE oficial (SIFEN v150, factura electrónica), lo sube a Storage y
 * actualiza `factura_electronica` (sin firma ni SET).
 *
 * Extraído del route `POST /api/facturas/[id]/sifen/xml` sin cambios de
 * comportamiento: el worker de la cola `sifen_jobs` necesita invocarlo
 * directamente (no tiene request ni cookies para pegarle por HTTP).
 */
