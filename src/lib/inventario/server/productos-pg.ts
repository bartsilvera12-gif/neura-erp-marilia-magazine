...
export interface SearchHitRow {
  ...
  // Compat con ferreteria (clasificación gastro/comercial — opcional)
...
export async function searchProductosPg(
  ...
  const sql = `
    SELECT p.id, p.nombre, p.sku, p.codigo_barras, p.codigo_barras_interno,
           p.precio_venta, p.costo_promedio, p.precio_minorista, p.precio_mayorista, p.stock_actual, p.stock_minimo,
