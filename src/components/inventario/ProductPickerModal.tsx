...
export interface ProductoPickerItem {
  ...
  // Compat con ferreteria (opcional, no siempre presente en variantes de ropa)
...
export interface AgregarVentaPayload {
  ...
  precio_input: number;
  iva: "EXENTA" | "5%" | "10%";
  /** Tipo de precio elegido en el panel de detalle. */
  tipo_precio: "minorista" | "mayorista" | "costo";
  // Compat con ferreteria (presentaciones — pack sizes / unidades de venta)
...
/**
 * Precio segun tipo (en Gs.):
 *  minorista -> precio_minorista (fallback precio_venta);
 *  mayorista -> precio_mayorista (fallback minorista, nunca 0 si hay minorista);
 *  costo     -> costo_promedio.
 * Decision Maria Cuevas: "Al costo" usa costo_promedio (CPP real mantenido por
 * compras/movimientos), NO precio_costo (costo estatico de alta).
 */
function precioPorTipoPicker(p: ProductoPickerItem, tipo: "minorista" | "mayorista" | "costo"): number {
...
export default function ProductPickerModal({
  ...
  function setPrecioEnMoneda(precioGs: number) {
  ...
  function selectProducto(p: ProductoPickerItem) {
    ...
    setPrecioEnMoneda(precioPorTipoPicker(p, "minorista"));
    setIva(ivaDefault);
    setFeedback(null);
  }
  ...
  function handleTipoPrecio(tipo: "minorista" | "mayorista" | "costo") {
    setTipoPrecio(tipo);
    if (sel) setPrecioEnMoneda(precioPorTipoPicker(sel, tipo));
  }
                ...
                {items.map((p) => {
                  ...
                  return (
                    <li
                      ...
                      } ${isSel ? "bg-[#4FAEB2]/15" : "hover:bg-slate-50"}`}
                    >
          ...
          <div className="hidden lg:flex w-2/5 flex-col overflow-y-auto bg-slate-50">
            {!sel ? (
              ...
              <div className="p-5 space-y-4">
                ...
                <h3 className="truncate text-sm font-semibold text-slate-800" title={sel.nombre}>{sel.nombre}</h3>
                    ...
                    <div className="flex border border-slate-200 rounded-lg overflow-hidden">
                      {(["minorista", "mayorista", "costo"] as const).map((t) => (
                        <button
                          ...
                          className={`flex-1 py-1.5 text-xs font-medium ${tipoPrecio === t ? "bg-[#4FAEB2] text-white" : "bg-white text-slate-600 hover:bg-slate-50"}`}
                        >
